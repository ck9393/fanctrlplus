#!/bin/bash
# fanctrlplus_loop.sh - 实际运行的风扇控制脚本（守护进程版）
...
pid_file="/var/run/${plugin}_${cfg_name}.pid"
rpm_file="/var/tmp/${plugin}/rpm_${cfg_name}"

mkdir -p /var/run /var/tmp/${plugin}
...
main_loop() {
  while true; do
    ...
    if [[ "$prev_pwm" == -1 || $(( pwm_val - prev_pwm >= 5 || prev_pwm - pwm_val >= 5 )) == 1 ]]; then
      ...
      logger -t $plugin "[$custom] Temp=${max_temp}°C → PWM=$pwm_val → RPM=$rpm"
      prev_pwm=$pwm_val
    fi

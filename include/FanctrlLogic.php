...
switch ($_GET['op'] ?? '') {
  ...
  case 'pause':
    ...
    if (is_file($pwm)) {
      ...
      $original_mode = file_exists($pwm_enable) ? trim(file_get_contents($pwm_enable)) : '2';

      file_put_contents($pwm_enable, "1");
      file_put_contents($pwm, "0");

      // 更稳定方式：使用 sleep + nohup 后台恢复
      $restore_cmd = "sleep 30 && echo $original_mode > '$pwm_enable' && echo $original_pwm > '$pwm'";

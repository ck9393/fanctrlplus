...
function render_fan_block($cfg, $i, $pwms, $disks) {
  ...
  <div class="fan-block" id="fan-block-<?=$i?>" data-index="<?=$i?>">
    <input type="hidden" class="cfg-file" name="file[<?=$i?>]" value="<?=htmlspecialchars($cfg['file'] ?? '')?>">
    ...
    <button type="button" id="apply-btn-<?=$i?>" onclick="applyFanBlock(<?=$i?>)" class="apply-block-btn">Apply</button>
    <button type="button" onclick="removeFan(this)" title="Delete this fan configuration" class="delete-block-btn">DELETE</button>
    ...
    <div class="fan-field">
      ...
      <div class="fan-pwm-row">
        ...
        <button type="button" onclick="pauseFan($(this).prev().val(), this)" class="pause-btn" title="Pause this fan for 30 seconds to identify its location.">Pause 30s</button>
      </div>
    </div>
    ...
    <div class="fan-field">
      ...
      <select class="disk-select" name="disks[<?=$i?>][]" multiple>
        <?php
